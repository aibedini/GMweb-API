...
| `POST /api/v1/agent/events/batch-v2` | Additive per-item upload: each index returns `ACCEPTED` with new sequence, `DUPLICATE` with original sequence, `CONFLICTING_DUPLICATE`, or `INVALID_EVENT` with a safe error code. `highWatermark` is the committed account cursor. V1 upload remains unchanged; V2 is not the preferred protocol until the full rollout is ready. | Agent signature or compatible device key |
...
| `GET /api/v1/web/snapshot-v2?token=&cursor=&limit=` | Continue the same snapshot by opaque cursor. Tokens are bound to the paired browser; wrong device is denied, expired or changed-generation snapshots require a new start. Delta sync begins after `baselineSequence` only after the final page is stored. | Linked session with `READ_MESSAGES` |
